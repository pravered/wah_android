...
class TextDrawer {
    ...
    public void draw(Canvas canvas, float x, float y, float offsetX, float offsetY, String textTarget) {

        float[] textPosition = getBestTextPosition();
        textPosition[0] = x;//TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, x, context.getResources().getDisplayMetrics());
        textPosition[1] = y;//TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, y, context.getResources().getDisplayMetrics());

        Log.d("Test1234", "in TextDrawer" + " text = " + textTarget);

        if (!TextUtils.isEmpty(mDetails)) {
            Log.d("Test1234", "in details textdrawer" + " text = " + textTarget);
            canvas.save();
...
//                float offsetForTitle = mDynamicTitleLayout != null ? mDynamicTitleLayout.getHeight() :0;
            ...
            if (mDynamicDetailLayout != null) {
                canvas.translate(textPosition[0] + TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, offsetX, context.getResources().getDisplayMetrics()), textPosition[1] + TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, offsetY, context.getResources().getDisplayMetrics()));
